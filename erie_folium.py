...
data1 = pd.read_csv("/Users/ericoliverio/Desktop/erie_covid.csv")
#-
...
for index, row in data1.iterrows():
    ...
    df = pd.DataFrame(data=[[int(row['Total Cases'])],[int(row['Daily Cases'])],[str(round(100*row['Total Cases']/row['Population'],1))+'%'],[round(row['GF'],1)]], columns=[str(row['Neighborhood'])+': '+str(row['ZIP'])],index=['Total Cases','Daily Cases','% Infected','Growth Factor'])
    #df.set_index('')
...
feature_group.add_child(plugins.HeatMap(stationArr,max_zoom=15,blur=33,radius=40))
